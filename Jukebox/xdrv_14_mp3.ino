/*
  ...
  1.0.0.6 20Jun2022 added   - Busy flag on optional GPIO pin
                    support - for MP3Folder, folder/track format
                    change  - to command decoding to allow MP3Folder 3,9 format
                            - 
  ...
  0.9.0.0 20180901  started - further development by mike2nl  - https://github.com/mike2nl/Sonoff-Tasmota
                  ...
                  #define D_SENSOR_DFR562_BUSY   "MP3 Player Busy"
...
enum MP3_Commands {                                 // commands useable in console or rules
  ...
  CMND_MP3_FOLDER,                                  // MP3Folder folder 01-99, track 0001...0255
...
char argument[XdrvMailbox.data_len];
  AddLog( LOG_LEVEL_INFO, PSTR("%s: %u"), "ArgC", ArgC());
  AddLog( LOG_LEVEL_INFO, PSTR("%s: %u"), "Payload", XdrvMailbox.payload );
  AddLog( LOG_LEVEL_INFO, PSTR("%s: %u"), "Payload Length", XdrvMailbox.data_len );
  AddLog( LOG_LEVEL_INFO, PSTR("%s: %u"), "Arg 1", strtol(ArgV(argument, 1), nullptr, 10) );
  AddLog( LOG_LEVEL_INFO, PSTR("%s: %u"), "Arg 2", strtol(ArgV(argument, 2), nullptr, 10) );
    ...
    switch (command_code) 
    {
      ...
      case CMND_MP3_DAC:
        ...
        if (XdrvMailbox.data_len > 0) 
        {
          ...
          if (command_code == CMND_MP3_FOLDER) { MP3_CMD(MP3_CMD_FOLDER, XdrvMailbox.payload); }
